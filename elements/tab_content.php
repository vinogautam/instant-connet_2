...
<div ng-if="tab.type == 'presentation'" class="col-xs-12 no-pad meeting-pane presentation-room thumbs-active" ng-init="currentpresentationindex='0'">
    <div class="col-xs-10 presentation-room no-pad h100">
      <ot-whiteboard  width="700" height="420"></ot-whiteboard>
      <img ng-src="{{'<?= IC_PLUGIN_URL; ?>/extract/'+tab.data.folder+'/'+tab.data.files[currentpresentationindex]}}" class="img-responsive absolute_center img_whm100">
    </div>

    <div class="col-xs-2 presentation-thumbs no-pad">

        <ul>
          <li ng-repeat="img in tab.data.files" ng-class="{active:currentpresentationindex==''+$index+''}" ng-click="$parent.currentpresentationindex=''+$index+''"><img ng-src="{{'<?= IC_PLUGIN_URL; ?>/extract/'+tab.data.folder+'/'+img}}" class="img-responsive"><p><span>Page {{$index+1}}</span></p></li>
           </ul> 
    </div>
  <div class="pane-footer col-xs-12">
     ...
     <div ng-hide="parseInt(currentpresentationindex)==0" class="per" ng-click="currentpresentationindex=''+(parseInt(currentpresentationindex)-1)+'';"><i class="fa fa-arrow-left" aria-hidden="true"></i> PREV</div>
     <div class="page-number">
      <select ng-model="currentpresentationindex">
        <option ng-repeat="img in tab.data.files" value="{{$index}}">Page {{$index+1}} of {{tab.data.files.length}}</option>
      </select>
     </div>
     <div ng-hide="parseInt(currentpresentationindex)==tab.data.files.length-1" class="next" ng-click="currentpresentationindex=''+(parseInt(currentpresentationindex)+1)+'';">NEXT <i class="fa fa-arrow-right" aria-hidden="true"></i></div>
     </div>
