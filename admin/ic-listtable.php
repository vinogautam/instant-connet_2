...
class WaitingEndoserTable extends WP_List_Table {
    ...
    function column_default($item, $column_name){
        ...
        switch($column_name){
            case 'status':
                return '<a href="?action=add_user&id='.$item['id'];
            case 'created':
                return date('Y/m/d', strtotime($item[$column_name]));
            default:
    ...
    function prepare_items() {
        ...
        $data = objectToArray($wpdb->get_results('select * from tmp_user where status = 0 order by id desc'));
        function usort_reorder($a,$b){
